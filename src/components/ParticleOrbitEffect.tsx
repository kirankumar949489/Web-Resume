import React, { useEffect, useRef, useState, useCallback } from "react";
  ...
  export interface ParticleOrbitEffectProps {
    ...
    // Add a prop to control whether it's full screen or container-bound
    fullScreen?: boolean;
  }
  
  // Define an interface for a single particle
  interface Particle {
    size: number;
  ...
  const ParticleOrbitEffect: React.FC<ParticleOrbitEffectProps> = ({
    className = "",
    style = {},
    fullScreen = false, // Default to false for container-bound behavior
  }) => {
    ...
    useEffect(() => {
      ...
      let canvasRect: DOMRect; // To store the canvas's position and dimensions
      ...
      const draw = () => {
        ...
        mouseRef.current.radiusScale = Math.min(
          mouseRef.current.radiusScale,
          RADIUS_SCALE_MAX
        );
        ...
        context.fillRect(
          0,
          0,
          mouseRef.current.width, // Use canvas's current width
        ...
        for (let i = 0, len = particlesRef.current.length; i < len; i++) {
          ...
          particle.position.x = Math.max(
            Math.min(particle.position.x, mouseRef.current.width),
            0
          );
          particle.position.y = Math.max(
            Math.min(particle.position.y, mouseRef.current.height),
            0
          );
    ...
    return (
      <canvas
        ...
        className={className}
        style={canvasStyle}
        width={canvasDimensions.width} // Set width based on state
        height={canvasDimensions.height} // Set height based on state
        aria-hidden="true"
        tabIndex={-1}
      />
    );
